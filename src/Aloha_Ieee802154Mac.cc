...
Define_Module(Aloha_Ieee802154Mac);

void  Aloha_Ieee802154Mac::initialize(int stage)
{
...
void  Aloha_Ieee802154Mac::finish()
{
...
Aloha_Ieee802154Mac::~Aloha_Ieee802154Mac()
{
...
void Aloha_Ieee802154Mac::configureInterfaceEntry()
{
 ...
 */
void Aloha_Ieee802154Mac::handleUpperPacket(Packet *packet)
{
    ...
    EV_DETAIL << "Aloha received a message from upper layer, name is " << packet->getName() << ", CInfo removed, mac addr=" << dest << endl;
...
void Aloha_Ieee802154Mac::updateStatusIdle(t_mac_event event, cMessage *msg)
{
    switch (event) {
...
void Aloha_Ieee802154Mac::updateStatusBackoff(t_mac_event event, cMessage *msg)
{
    switch (event) {
...
void Aloha_Ieee802154Mac::attachSignal(Packet *mac, simtime_t_cref startTime)
{
    simtime_t duration = mac->getBitLength() / bitrate;
    mac->setDuration(duration);
}

void Aloha_Ieee802154Mac::updateStatusCCA(t_mac_event event, cMessage *msg)
{
    switch (event) {
        case EV_TIMER_CCA: {
            bool isIdle = radio->getReceptionState() == IRadio::RECEPTION_STATE_IDLE;
            if (!isIdle) {
                EV_DETAIL << "Canale non libero" << endl;
            }
...
void Aloha_Ieee802154Mac::updateStatusTransmitFrame(t_mac_event event, cMessage *msg)
{
...
void Aloha_Ieee802154Mac::updateStatusWaitAck(t_mac_event event, cMessage *msg)
{
    assert(useMACAcks);
...
void Aloha_Ieee802154Mac::manageMissingAck(t_mac_event    /*event*/, cMessage *    /*msg*/)
{
...
void Aloha_Ieee802154Mac::updateStatusSIFS(t_mac_event event, cMessage *msg)
{
    assert(useMACAcks);
...
void Aloha_Ieee802154Mac::updateStatusTransmitAck(t_mac_event event, cMessage *msg)
{
    assert(useMACAcks);
...
void Aloha_Ieee802154Mac::updateStatusNotIdle(cMessage *msg)
{
 ...
 */
void Aloha_Ieee802154Mac::executeMac(t_mac_event event, cMessage *msg)
{
...
void Aloha_Ieee802154Mac::manageQueue()
{
...
void Aloha_Ieee802154Mac::updateMacState(t_mac_states newMacState)
{
 ...
 */
void Aloha_Ieee802154Mac::fsmError(t_mac_event event, cMessage *msg)
{
...
void Aloha_Ieee802154Mac::startTimer(t_mac_timer timer)
{
...
simtime_t Aloha_Ieee802154Mac::scheduleBackoff()
{
    return  simTime();
}
 ...
 */
void Aloha_Ieee802154Mac::handleSelfMessage(cMessage *msg)
{
 ...
 */
void Aloha_Ieee802154Mac::handleLowerPacket(Packet *packet)
{
...
//            nbRxFrames++;
            ...
            if (strcmp(packet->getName(), "Aloha-Ack") != 0) {
...
//                nbRxFrames++;
                ...
                ackMessage = new Packet("Aloha-Ack");
...
void Aloha_Ieee802154Mac::receiveSignal(cComponent *source, simsignal_t signalID, intval_t value, cObject *details)
{
    Enter_Method_Silent();
    ...
    if (signalID == IRadio::transmissionStateChangedSignal) {
        ...
        if (newRadioTransmissionState == IRadio::TRANSMISSION_STATE_TRANSMITTING) {
                          lastTxTime = simTime(); // Tempo in cui inizia a trasmettere
        }else if(transmissionState == IRadio::TRANSMISSION_STATE_TRANSMITTING){
                      EV << "LAST time = " << lastTxTime<< " simTime ="<< simTime() <<endl;
                          simtime_t tx = simTime() - lastTxTime; //Tempo di trasmissione
                          txTime += tx;
                          EV << "TX TIME= " << txTime<< endl;
                          simsignal_t txSignalId = registerSignal("txTimeChanged");
                          emit(txSignalId,tx);
        }
    ...
    if (signalID == IRadio::receptionStateChangedSignal) {
        IRadio::ReceptionState newRadioReceptionState = static_cast<IRadio::ReceptionState>(value);
          if (newRadioReceptionState == IRadio::RECEPTION_STATE_RECEIVING) {
                  lastRxTime = simTime(); // Tempo in cui inizia a ricevere
          }else if(receptionState == IRadio::RECEPTION_STATE_RECEIVING){
              EV << "LAST time = " << lastRxTime<< " simTime ="<< simTime() <<endl;
                  simtime_t rx = simTime() - lastRxTime; //Tempo di ricezione
                  rxTime += rx;
                  EV << "RX TIME= " << rxTime<< endl;
                  simsignal_t rxSignalId = registerSignal("rxTimeChanged");
                  emit(rxSignalId,rx);
          }
          if (newRadioReceptionState == IRadio::RECEPTION_STATE_BUSY) {
                           lastBusyRxTime = simTime();
          }else if(receptionState == IRadio::RECEPTION_STATE_BUSY){

                   simtime_t busy = simTime() - lastBusyRxTime;
                   rxTime += busy;
                   EV << "RX TIME= " << rxTime<< endl;
                   simsignal_t rxSignalId = registerSignal("rxTimeChanged");
                   emit(rxSignalId,busy);
          }
        receptionState = newRadioReceptionState;
    }
    simtime_t idle=simTime()-txTime- rxTime;
    simsignal_t idleSignalId = registerSignal("idleTimeChanged");
    emit(idleSignalId,idle);
}

void Aloha_Ieee802154Mac::decapsulate(Packet *packet)
{
